...
int main(void)
{
	...
	if (ans == 0) {
        printf("CV\n");
        Values my_tri = CV_findValues(); // Declare my_tri variable here
		CV_ex(&my_frame, my_point, my_tri);
	}
    else if (ans == 1){
        printf("DPV\n");
        Values my_pulse = DPV_findValues(); // Declare my_tri variable here
        DPV_ex(&my_frame,my_point, my_pulse);
    }
    else if(ans ==2){
        printf("SW\n");
        Values my_square = SqW_findValues(); // Declare my_tri variable here
