...
void graph(float** memory , float i, float v1,float comp1,float v2,float comp2,float val){
    ...
    else if(compare_Values(v1,comp1) == 1 && compare_Values(v2,comp2)){
        v2 = v2-val;
        SW_mem(memory, i, v1, v2);
...
int main (){
    ...
    FILE *ptr_fp;
    configSq values;
    int row = 135;//(values.period * ((values.amount*2)-1)) + ((rise * ((values.amount*2)-1))*2 + (values.incre*((values.amount*2)-1)));
    int col = 2;
